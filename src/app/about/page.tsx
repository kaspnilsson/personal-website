...
export default function AboutPage() {
  return (
    <main className="font-mono min-h-screen px-4 py-10 sm:px-20 sm:py-16">
      <div className="max-w-5xl mx-auto">
        <header className="mb-10 sm:mb-14">
          <div className="title-eyebrow">Electronic Music Producer / DJ</div>
          <h1 className="display heading-tight">kasp</h1>
          <div className="title-rule" />
          <div className="text-xs font-mono text-[var(--renders-muted-foreground)] flex flex-wrap gap-2 mt-3">
            <span className="bg-secondary border border-border px-2 py-1">Speed Garage</span>
            <span className="bg-secondary border border-border px-2 py-1">UK 2‑Step</span>
            <span className="bg-secondary border border-border px-2 py-1">Club</span>
          </div>
        ...
        <div className="flex flex-col gap-8 sm:gap-10">
          ...
          <SectionCard title="About">
            <div className="grid sm:grid-cols-3 gap-6">
              <div className="sm:col-span-2 text-sm/6 text-[var(--renders-muted-foreground)]">
              ...
              <ul className="text-sm/6 text-[var(--renders-muted-foreground)] space-y-2">
                <li><span className="font-mono text-xs tracking-widest uppercase mr-2">Focus</span> Speed Garage / UK 2‑Step</li>
                <li><span className="font-mono text-xs tracking-widest uppercase mr-2">Origin</span> Brooklyn, NY</li>
                <li><span className="font-mono text-xs tracking-widest uppercase mr-2">BPM</span> 132–138</li>
              </ul>
            </div>
          </SectionCard>
          ...
          <SectionCard title="Support Credits">
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {[
                ...
                <span key={name} className="text-xs font-mono text-[var(--renders-muted-foreground)] bg-secondary border border-border px-2 py-1">
                  {name}
                </span>
              ))}
            </div>
          ...
          <SectionCard title="Contact">
            ...
            {!!(socials.aliases && socials.aliases.length) && (
              <div className="mt-4 text-xs font-mono text-[var(--renders-muted-foreground)] flex flex-wrap gap-2">
                {socials.aliases!.map((alias) => (
                  <span key={alias} className="bg-secondary border border-border px-2 py-1">{alias}</span>
                ))}
              </div>
            )}
          </SectionCard>
  ...
  return (
    <div className="border-b border-border pb-4 last:border-b-0">
      <div className="flex flex-col gap-2.5">
        ...
        <div className="text-xs text-[var(--renders-muted-foreground)] font-mono tracking-widest uppercase">{meta}</div>
      </div>
    </div>
  );
}
